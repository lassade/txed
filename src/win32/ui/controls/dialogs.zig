...
pub const IPrintDialogCallback = extern struct {
    ...
    pub fn MethodMixin(comptime T: type) type {
        return struct {
            ...
            pub inline fn handleMessage(self: *const T, h_dlg_: ?HWND, u_msg_: u32, w_param_: WPARAM, l_param_: LPARAM, p_result_: ?*LRESULT) HRESULT {
                return @as(*const IPrintDialogCallback.VTable, @ptrCast(self.vtable)).HandleMessage(@as(*const IPrintDialogCallback, @ptrCast(self)), h_dlg_, u_msg_, w_param_, l_param_, p_result_);
            }
        };
    }
...
pub const IPrintDialogServices = extern struct {
    ...
    pub fn MethodMixin(comptime T: type) type {
        return struct {
            pub usingnamespace IUnknown.MethodMixin(T);
            pub inline fn getCurrentDevMode(self: *const T, p_dev_mode_: ?*DEVMODEA, pcb_size_: ?*u32) HRESULT {
                return @as(*const IPrintDialogServices.VTable, @ptrCast(self.vtable)).GetCurrentDevMode(@as(*const IPrintDialogServices, @ptrCast(self)), p_dev_mode_, pcb_size_);
            }
            pub inline fn getCurrentPrinterName(self: *const T, p_printer_name_: ?[*:0]u16, pcch_size_: ?*u32) HRESULT {
                return @as(*const IPrintDialogServices.VTable, @ptrCast(self.vtable)).GetCurrentPrinterName(@as(*const IPrintDialogServices, @ptrCast(self)), p_printer_name_, pcch_size_);
            }
            pub inline fn getCurrentPortName(self: *const T, p_port_name_: ?[*:0]u16, pcch_size_: ?*u32) HRESULT {
                return @as(*const IPrintDialogServices.VTable, @ptrCast(self.vtable)).GetCurrentPortName(@as(*const IPrintDialogServices, @ptrCast(self)), p_port_name_, pcch_size_);
            }
        };
    }
    pub usingnamespace MethodMixin(@This());
