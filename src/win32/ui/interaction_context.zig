...
pub const INTERACTION_FLAGS = enum(u32) {
    ...
    pub fn initFlags(o: struct {
        ...
        return @as(INTERACTION_FLAGS, @enumFromInt((if (o.NONE == 1) @intFromEnum(INTERACTION_FLAGS.NONE) else 0) | (if (o.BEGIN == 1) @intFromEnum(INTERACTION_FLAGS.BEGIN) else 0) | (if (o.END == 1) @intFromEnum(INTERACTION_FLAGS.END) else 0) | (if (o.CANCEL == 1) @intFromEnum(INTERACTION_FLAGS.CANCEL) else 0) | (if (o.INERTIA == 1) @intFromEnum(INTERACTION_FLAGS.INERTIA) else 0) | (if (o.MAX == 1) @intFromEnum(INTERACTION_FLAGS.MAX) else 0)));
    }
};
...
pub extern "ninput" fn CreateInteractionContext(
    interaction_context: ?*?HINTERACTIONCONTEXT,
...
pub extern "ninput" fn RegisterOutputCallbackInteractionContext(
    interaction_context: ?HINTERACTIONCONTEXT,
    output_callback: ?INTERACTION_CONTEXT_OUTPUT_CALLBACK,
    client_data: ?*anyopaque,
) callconv(@import("std").os.windows.WINAPI) HRESULT;

pub extern "ninput" fn RegisterOutputCallbackInteractionContext2(
    interaction_context: ?HINTERACTIONCONTEXT,
    output_callback: ?INTERACTION_CONTEXT_OUTPUT_CALLBACK2,
    client_data: ?*anyopaque,
) callconv(@import("std").os.windows.WINAPI) HRESULT;
...
pub extern "ninput" fn SetPropertyInteractionContext(
    interaction_context: ?HINTERACTIONCONTEXT,
    context_property: INTERACTION_CONTEXT_PROPERTY,
    value: u32,
) callconv(@import("std").os.windows.WINAPI) HRESULT;
...
pub extern "ninput" fn SetCrossSlideParametersInteractionContext(
    interaction_context: ?HINTERACTIONCONTEXT,
    parameter_count: u32,
    cross_slide_parameters: [*]CROSS_SLIDE_PARAMETER,
) callconv(@import("std").os.windows.WINAPI) HRESULT;
