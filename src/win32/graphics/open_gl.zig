...
pub const PFD_FLAGS = enum(u32) {
    ...
    pub fn initFlags(o: struct {
        ...
        return @as(PFD_FLAGS, @enumFromInt((if (o.DOUBLEBUFFER == 1) @intFromEnum(PFD_FLAGS.DOUBLEBUFFER) else 0) | (if (o.STEREO == 1) @intFromEnum(PFD_FLAGS.STEREO) else 0) | (if (o.DRAW_TO_WINDOW == 1) @intFromEnum(PFD_FLAGS.DRAW_TO_WINDOW) else 0) | (if (o.DRAW_TO_BITMAP == 1) @intFromEnum(PFD_FLAGS.DRAW_TO_BITMAP) else 0) | (if (o.SUPPORT_GDI == 1) @intFromEnum(PFD_FLAGS.SUPPORT_GDI) else 0) | (if (o.SUPPORT_OPENGL == 1) @intFromEnum(PFD_FLAGS.SUPPORT_OPENGL) else 0) | (if (o.GENERIC_FORMAT == 1) @intFromEnum(PFD_FLAGS.GENERIC_FORMAT) else 0) | (if (o.NEED_PALETTE == 1) @intFromEnum(PFD_FLAGS.NEED_PALETTE) else 0) | (if (o.NEED_SYSTEM_PALETTE == 1) @intFromEnum(PFD_FLAGS.NEED_SYSTEM_PALETTE) else 0) | (if (o.SWAP_EXCHANGE == 1) @intFromEnum(PFD_FLAGS.SWAP_EXCHANGE) else 0) | (if (o.SWAP_COPY == 1) @intFromEnum(PFD_FLAGS.SWAP_COPY) else 0) | (if (o.SWAP_LAYER_BUFFERS == 1) @intFromEnum(PFD_FLAGS.SWAP_LAYER_BUFFERS) else 0) | (if (o.GENERIC_ACCELERATED == 1) @intFromEnum(PFD_FLAGS.GENERIC_ACCELERATED) else 0) | (if (o.SUPPORT_DIRECTDRAW == 1) @intFromEnum(PFD_FLAGS.SUPPORT_DIRECTDRAW) else 0) | (if (o.DIRECT3D_ACCELERATED == 1) @intFromEnum(PFD_FLAGS.DIRECT3D_ACCELERATED) else 0) | (if (o.SUPPORT_COMPOSITION == 1) @intFromEnum(PFD_FLAGS.SUPPORT_COMPOSITION) else 0) | (if (o.DEPTH_DONTCARE == 1) @intFromEnum(PFD_FLAGS.DEPTH_DONTCARE) else 0) | (if (o.DOUBLEBUFFER_DONTCARE == 1) @intFromEnum(PFD_FLAGS.DOUBLEBUFFER_DONTCARE) else 0) | (if (o.STEREO_DONTCARE == 1) @intFromEnum(PFD_FLAGS.STEREO_DONTCARE) else 0)));
    }
};
...
pub extern "glu32" fn gluProject(
    ...
    objy: f64,
    objz: f64,
    model_matrix: ?*const f64,
    proj_matrix: ?*const f64,
    viewport: ?*const i32,
    winx: ?*f64,
    winy: ?*f64,
    winz: ?*f64,
